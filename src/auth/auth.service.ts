...
return {
   ...
   */
    async loginWithGoogle(
      ...
      if(!user){
        const hashedPassword = await hash(password, hashConfig.saltRounds);
    const newUsr=    await this.prismaService.user.create({
          data:{
            email:email,
            firstName:firstName,
            lastName:lastName,
            avatar:avatar,
            role:role,
            referralId:role==Role.AGENT?password:'',
            isVerified:true,
      ...
      const payload = { sub: newUsr.id, userRole: newUsr.role };
